...
def openModalGetDataCloseModalPerRow():
    ...
    result = driver.execute_script(
        ...
        for (var i of document.querySelectorAll('.modal#viewModalPPH')){
            data_pendamping.push({
                ...
                kabupaten: i.querySelector('span#lblKabupatenPendamping').textContent,  // Added kabupaten
                kecamatan: i.querySelector('span#lblKecamatanPendamping').textContent,  // Added kecamatan
    ...
    data_pendamping_halal.append({
        ...
        "kabupaten": result[0]['kabupaten'],  # Store kabupaten
        "kecamatan": result[0]['kecamatan'],  # Store kecamatan
        "no_registrasi": result[0]['no_registrasi'],
        "tgl_terbit": result[0]['tgl_terbit']
    })

    sql = "INSERT INTO data_pph_province (province, email, name, no_telp, kabupaten, kecamatan, pendampingan_pelaku_usaha, no_registrasi, tgl_terbit) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE name = VALUES(name), no_telp = VALUES(no_telp), kabupaten = VALUES(kabupaten), kecamatan = VALUES(kecamatan), pendampingan_pelaku_usaha = VALUES(pendampingan_pelaku_usaha),no_registrasi = VALUES(no_registrasi),  tgl_terbit = VALUES(tgl_terbit)"
    
    val = (nameProv, result[0]['email'], result[0]['name'], result[0]['no_telp'], result[0]['kabupaten'], result[0]['kecamatan'], None, result[0]['no_registrasi'], result[0]['tgl_terbit'])
   ...
   # Click close modal button
    ...
    if visible_buttons:  # Check if there are any visible buttons
      btn_lihat_click = visible_buttons[-1]  # Safely access the last visible button
      driver.execute_script("arguments[0].click();", btn_lihat_click)
      time.sleep(1)  # Optionally, wait for modal to close
    else:
